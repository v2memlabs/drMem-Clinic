...
void main() {
  ...
  Future<void> pumpEncounterPdfForm(WidgetTester tester) async {
    AuthSession.setUser(
      AppUser(
        id: 'd1',
        username: 'doc',
    ...
    final router = GoRouter(
      initialLocation:
          '/pdf-outputs/new?patientId=p1&source=$pdfSourceModuleClinicalEncounter&id=ce1',
      routes: [
        GoRoute(
  ...
  testWidgets('clinical encounter async prefill fills form without not-found', (
    tester,
  ) async {
    await pumpEncounterPdfForm(tester);

    expect(find.textContaining('Kaynak: Muayene Kayıtları'), findsOneWidget);
    expect(
      find.textContaining('Kaynak muayene kaydı bulunamadı'),
      findsNothing,
    );
